...
MAT_TEMPLATE inline void
matrixT::getAbsoluteValue()
{
    for (size_t i=0; i < _m->Row; i++)
      for (size_t j=0; j < _m->Col; j++)
          _m->Val[i][j] = abs(_m->Val[i][j] );
}

MAT_TEMPLATE inline float
matrixT::getMaxElement()
{
    float max = -1e18;
    for (size_t i=0; i < _m->Row; i++)
      for (size_t j=0; j < _m->Col; j++)
...
MAT_TEMPLATE inline void
matrixT::getSquaredValue()
{
    for (size_t i=0; i < _m->Row; i++)
      for (size_t j=0; j < _m->Col; j++)
          _m->Val[i][j] = _m->Val[i][j] * _m->Val[i][j];
}
MAT_TEMPLATE inline void
 matrixT::setAllZeros(){
    for (size_t i=0; i < _m->Row; i++)
