...
public:
	...
	inline friend Vector3 operator *(const float fScalar,
	...
	/** Calculates the dot (scalar) product of this vector with another.
	 ...
	 @param
	 vec Vector3 with which to calculate the dot product (together
	 with this one).
	 @returns
	...
	/** Calculates the absolute dot (scalar) product of this vector with another.
	 ...
	 @param
	 vec Vector3 with which to calculate the absolute dot product (together
	 with this one).
	 @returns
	...
	/** Calculates the cross-product of 2 vectors, i.e. the vector that
	 ...
	 of a triangle).
	 @param
	 vec Vector3 which, together with this one, will be used to
	...
	inline Vector3 midPoint(const Vector3& vec) const {
		return Vector3((x + vec.x) * 0.5f, (y + vec.y) * 0.5f, (z + vec.z)
				* 0.5f);
	}
	...
	inline bool operator >=(const Vector3& rhs) const {
		if (x >= rhs.x && y >= rhs.y && z >= rhs.z)
			return true;
		return false;
	} //thought comparing two floats to be equal are not meaningful all the time,
	// sometimes we did need them in special cases.
	...
	/** Generates a 2D vector by project this 3D vector onto a specific
	 dimension.
	 */

	inline Vector2 project(int dim = 2) const {
		if (dim == 0)
			return Vector2(y, z);
		if (dim == 1)
			return Vector2(x, z);
	...
	inline Vector3 spherical() {
		Vector3 temp;
		float x2ny2 = x*x + y*y;
		temp.x = atan2(z, sqrt( x2ny2) ); // zenith angle, i.e. theta
		// I didn't use atan(z/sqrt(x2ny2)) to make the restriction to avoid div-by-zero troubles
		temp.y = atan2(y, x);// azimuth angle, i.e. phi
		temp.z = sqrt(x2ny2 + z*z);// radius, i.e. r
		return temp;
	}

	inline void cart2spherical() {
		*this = this->spherical();
	}
	...
	inline bool positionEquals(const Vector3& rhs, float tolerance = 1e-03) const {
		return fabs(x - rhs.x) < tolerance && fabs(y - rhs.y) < tolerance
	 ...
	 */
	inline bool directionEquals(const Vector3& rhs, const float& tolerance) const {
	 ...
	 */
	inline _OgreExport friend std::ostream& operator <<(std::ostream& o,
			const Vector3& v) {
		o << "Vector3(" << v.x << ", " << v.y << ", " << v.z << ")";
		return o;
	}

	inline _OgreExport friend std::istream& operator >>(std::istream& in,
			Vector3& v) {
		in >> v.x >> v.y >> v.z;
		return in;
	}
};
Vector3 vmax(const Vector3& v1, const Vector3& v2);
Vector3 vmin(const Vector3& v1, const Vector3& v2);
Vector3 vabs(const Vector3& v);

inline bool equal(const Vector3& v1, const Vector3& v2) {
	return abs(v1.x - v2.x) < eps && abs(v1.y - v2.y) < eps && abs(v1.z - v2.z)
			< eps;
}
